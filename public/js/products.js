...
const buttonPressed = (e) => {
  ...
  <input type="text" placeholder="Yetkazib berish manzili.." name="address">
  </lable>
  </div>
  <button id="buy">Sotib olish</button>
  </div>`;
...
function applyDiscount(button) {
  const priceContainer = button.querySelector(".price p");
  if (priceContainer) {
    const originalPrice = "7 999 000 so'm"; // Eski narx
    const discountedPrice = "2 999 000 so'm"; // Yangi narx
    priceContainer.innerHTML = `
      <span style="text-decoration: line-through; color: red;">${originalPrice}</span>
      <br>
      <span style="color: green; font-weight: bold;">${discountedPrice}</span>
    `;
  }
}
...
function showSuggestions(arr) {
  ...
  if (!arr.length) {
    ...
    arrData = "<li>" + inputValue + " - topilmadi 🙁</li>";
  } else {
    //join the arry
    arrData = arr.join("");
  }
...
async function loadProducts() {
    try {
        ...
        productsHolder.innerHTML = products.map(product => `
            <button class="products_card_l" data-id="${product._id}">
                <div class="product_img">
                    <i><img src="https://img.icons8.com/cotton/64/000000/hearts---v1.png"/></i>
                    <img src="${product.image_url || '../images/Leather-wallet.jpeg'}">
                </div>
        ...
        // Mahsulot kartochkalariga click event qo'shish
        const productCards = document.getElementsByClassName("products_card_l");
        for (let button of productCards) {
            button.addEventListener("click", buttonPressed);
        }
